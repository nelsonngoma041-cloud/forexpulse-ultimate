...
async function derivWS(messages: WSMsg[]): Promise<WSMsg[]> {
  const { WebSocket } = await import('ws');
  return new Promise((resolve, reject) => {
    const ws = new WebSocket(DERIV_WS);
    const responses: WSMsg[] = [];
    let idx = 0;
    const t = setTimeout(() => { ws.close(); reject(new Error('Deriv timeout')); }, 15000);
    ...
    ws.on('message', (data: Buffer) => {
      ...
      else { clearTimeout(t); ws.close(); resolve(responses); }
    });
...
async function derivBuy(
  ...
  try {
    ...
    const propRes = await derivWS([
      { authorize: token },
      { proposal:1, amount:stake, basis:'stake', contract_type: action==='BUY'?'CALL':'PUT',
    ...
    if (!prop) return { ok:false, error:'No proposal from Deriv' };
...
async function sendTelegram(text: string): Promise<{ok:boolean;error?:string}> {
  ...
  try {
    ...
    if(!data.ok) { console.error('Telegram rejected:',JSON.stringify(data)); return {ok:false,error:data.description}; }
    return {ok:true};
  } catch(err) {
...
async function dispatchSignal(stake=10, durationMin=5) {
  ...
  // ── Auto-execute on Deriv if token is set ────────────────────────────────
  ...
  if (derivToken) {
    ...
    if (tradeResult.ok) {
      ...
      executionLine = `❌ Deriv execution failed: ${md(tradeResult.error??'unknown')}`;
    }
  }
  ...
  if (sent.ok) {
    ...
    recentSignals.unshift({
      id:`${Date.now()}`, symbol:display, action:sig.action,
      confidence:sig.confidence, rsi:sig.rsi, time:time.slice(0,5),
      executed: tradeResult.ok, contractId: tradeResult.contractId,
    });
    if (recentSignals.length>20) recentSignals.pop();
    console.log(`Signal #${signalCount}: ${sig.action} ${display} | Deriv: ${tradeResult.ok?`contract ${tradeResult.contractId}`:'manual'}`);
  }
}

function startBot(stake:number, duration:number) {
  isRunning=true; signalCount=0; executedCount=0;
...
export async function GET() {
  ...
  return NextResponse.json({
    ...
    recentSignals,
    message: isRunning?`Running — ${signalCount} signals, ${executedCount} executed`:'Bot stopped',
    debug: {
      hasToken: !!token,
...
export async function POST(request: Request) {
  ...
  if (action==='start') {
    ...
    const result = await sendTelegram(
      ...
      `${derivToken?'✅ Deriv auto\\-execution: ON':'⚠️ Manual mode — Deriv token not set'}\n` +
  ...
  if (action==='test') {
    ...
    // Test Deriv connection without placing a real trade
    let derivStatus = 'Not configured — add DERIV_API_TOKEN to Vercel';
    if (derivToken) {
      try {
        const responses = await derivWS([
          {authorize:derivToken},
          {balance:1,account:'current'},
        ]);
        const bal = (responses[1] as {balance?:{balance:number;currency:string}}).balance;
        derivStatus = bal ? `Connected ✓ — Balance: ${bal.currency} ${bal.balance.toFixed(2)}` : 'Connected but no balance data';
      } catch (err) {
        derivStatus = `Error: ${err instanceof Error?err.message:String(err)}`;
      }
    }
    ...
    return NextResponse.json({
      success: result.ok,
      message: result.ok?'Test signal sent':result.error,
