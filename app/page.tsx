...
import { Bot, MessageCircle, Play, Pause, Activity, Settings, TrendingUp, TrendingDown, Shield, Wifi, WifiOff, DollarSign, ChevronRight, ChevronLeft } from "lucide-react";
import { TelegramAlertBot, TradeAlert } from './lib/telegram-alerts';

// Initialize Telegram bot with YOUR NEW working token
        ...
        <nav className="p-2">
          {['dashboard', 'settings'].map(tab => (
            <button 
              ...
              className={`w-full flex items-center gap-3 px-3 py-2 rounded mb-1 transition-all ${activeTab === tab ? 'bg-gray-800 text-white' : 'text-gray-400 hover:bg-gray-800/50 hover:text-white'}`}
            >
        ...
        <header className="sticky top-0 z-30 border-b border-gray-800 bg-gray-950/95 backdrop-blur-xl px-6 py-3 flex justify-between items-center">
          ...
          <div className="flex gap-2">
            <button 
              ...
              className="bg-cyan-500/20 text-cyan-400 hover:bg-cyan-500/30 px-3 py-1.5 rounded-lg flex items-center gap-2 text-sm transition-all"
            >
              <MessageCircle className="w-4 h-4" /> Test Alert
            </button>
            <button 
              onClick={toggleBot} 
              className={`px-4 py-1.5 rounded-lg flex items-center gap-2 text-sm transition-all ${
                botRunning ? 'bg-red-500/20 text-red-400 hover:bg-red-500/30' : 'bg-emerald-500/20 text-emerald-400 hover:bg-emerald-500/30'
              }`}
            >
        ...
        <div className="p-6">
          ...
          {activeTab === 'dashboard' && (
            <div className="text-center py-20">
              ...
              <button 
                onClick={sendTestAlert} 
                className="bg-emerald-500/20 text-emerald-400 hover:bg-emerald-500/30 px-6 py-3 rounded-lg transition-all font-medium"
              >
              ...
              <div className="bg-gray-900 rounded-xl p-6 border border-gray-800">
                ...
                <div className="space-y-3">
                  <div className="flex justify-between py-2 border-b border-gray-800">
                    ...
                    <span className="text-green-400 font-mono text-sm">✓ Configured</span>
                  </div>
                  <div className="flex justify-between py-2 border-b border-gray-800">
                  ...
                  <div className="flex justify-between py-2 border-b border-gray-800">
                    ...
                    <span className="text-blue-400">@SamuelTradingBot</span>
                  </div>
                ...
                <div className="mt-6 p-3 bg-gray-800/50 rounded-lg">
                  <p className="text-sm text-gray-300">📌 To receive alerts:</p>
                  <ol className="text-xs text-gray-400 list-decimal list-inside mt-2 space-y-1">
                    <li>Search for <span className="text-blue-400">@SamuelTradingBot</span> on Telegram</li>
                    <li>Click the <span className="text-green-400">"Start"</span> button</li>
                    <li>Type <span className="text-yellow-400">/start</span></li>
                    <li>Click <span className="text-cyan-400">"Test Alert"</span> on this dashboard</li>
                  </ol>
                </div>
              </div>
            </div>
