...
import { 
  ...
  BarChart3, History, Calendar, TrendingUp as TrendingUpIcon,
  Award, Star, AlertCircle, PieChart
} from "lucide-react";
import { 
...
export default function Home() {
  ...
  const [performanceData, setPerformanceData] = useState([
    { day: 'Mon', pnl: 120, trades: 3 },
    { day: 'Tue', pnl: -80, trades: 2 },
    { day: 'Wed', pnl: 200, trades: 4 },
    { day: 'Thu', pnl: 150, trades: 3 },
    { day: 'Fri', pnl: -50, trades: 1 },
    { day: 'Sat', pnl: 300, trades: 2 },
    { day: 'Sun', pnl: 100, trades: 1 },
  ]);
  
  const [tradeHistory, setTradeHistory] = useState<TradeHistory[]>([
    { id: '1', symbol: 'EUR/USD', action: 'BUY', entryPrice: 1.0850, exitPrice: 1.0920, pnl: 70, entryTime: new Date().toISOString() },
    { id: '2', symbol: 'GBP/USD', action: 'BUY', entryPrice: 1.2670, exitPrice: 1.2750, pnl: 80, entryTime: new Date().toISOString() },
    { id: '3', symbol: 'USD/JPY', action: 'SELL', entryPrice: 157.20, exitPrice: 156.50, pnl: 70, entryTime: new Date().toISOString() },
  ]);
  ...
  const bestTrade = Math.max(...tradeHistory.map(t => t.pnl), 0);
  const worstTrade = Math.min(...tradeHistory.map(t => t.pnl), 0);
  const avgTrade = totalTrades ? (tradeHistory.reduce((sum, t) => sum + t.pnl, 0) / totalTrades) : 0;
  const profitFactor = tradeHistory.filter(t => t.pnl > 0).reduce((sum, t) => sum + t.pnl, 0) / Math.abs(tradeHistory.filter(t => t.pnl < 0).reduce((sum, t) => sum + t.pnl, 0)) || 0;
  ...
  const exportData = () => {
    const data = { positions, newsFeed, tradeHistory, performanceData, exportDate: new Date().toISOString() };
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-gray-200">
      ...
      <aside className={`fixed left-0 top-0 h-full transition-all duration-300 bg-gray-950 border-r border-gray-800 z-40 ${sidebarCollapsed ? 'w-16' : 'w-64'}`}>
        ...
        <nav className="p-2">
          {['dashboard', 'analytics', 'history', 'settings'].map(tab => (
        ...
        <div className="p-6">
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                ...
                <div className="rounded-xl bg-gray-900 border border-gray-800 p-4">
                  ...
                  <div className="text-2xl font-bold text-emerald-400">{profitFactor.toFixed(2)}</div>
                </div>
              </div>
          ...
          {activeTab === 'analytics' && (
            <div className="space-y-6">
              ...
              <div className="rounded-xl bg-gray-900 border border-gray-800 p-4">
                <h3 className="text-sm text-gray-400 mb-4">Daily P&L Performance</h3>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="rounded-xl bg-gray-900 border border-gray-800 p-4">
                  <h3 className="text-sm text-gray-400 mb-3">Best Trade</h3>
                  <div className="text-2xl font-bold text-emerald-400">+${bestTrade}</div>
                </div>
                <div className="rounded-xl bg-gray-900 border border-gray-800 p-4">
                  <h3 className="text-sm text-gray-400 mb-3">Worst Trade</h3>
                  <div className="text-2xl font-bold text-red-400">${worstTrade}</div>
                </div>
                <div className="rounded-xl bg-gray-900 border border-gray-800 p-4">
                  <h3 className="text-sm text-gray-400 mb-3">Average Trade</h3>
                  <div className={`text-2xl font-bold ${avgTrade >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>${avgTrade.toFixed(0)}</div>
                </div>
                <div className="rounded-xl bg-gray-900 border border-gray-800 p-4">
                  <h3 className="text-sm text-gray-400 mb-3">Expectancy</h3>
                  <div className="text-2xl font-bold text-blue-400">${(avgTrade * winRate / 100).toFixed(0)}</div>
                </div>
              </div>
            </div>
          )}
          ...
          {activeTab === 'history' && (
            <div className="rounded-xl bg-gray-900 border border-gray-800 overflow-hidden">
              <div className="px-4 py-3 border-b border-gray-800">
                <h3 className="font-medium flex items-center gap-2"><History className="w-4 h-4 text-emerald-400" /> Trade History</h3>
              </div>
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead className="bg-gray-800/50">
              ...
              <div className="bg-gray-900 rounded-xl p-6 border border-gray-800">
                ...
                <div className="mt-6 p-3 bg-gray-800/50 rounded-lg">
                  ...
                  <ol className="text-xs text-gray-400 list-decimal list-inside mt-2 space-y-1">
                    ...
                    <li>Check <span className="text-cyan-400">"Analytics"</span> tab to track performance</li>
                  </ol>
                </div>
              </div>
            </div>
